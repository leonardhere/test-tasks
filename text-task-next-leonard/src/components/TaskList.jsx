...
const TaskList = ({markCompleted,deleteTask,updateTask}) => {
  const [toDoHeading, setToDoHeading] = useState('Нет задач в списке')
  ...
  useEffect(()=>{
    if(taskList?.filter(task=>task.completed===true)?.length>0){
      setToDoCompletedHeading('Список завершённых задач')
    }else{
  ...
  useEffect(()=>{
    if(taskList?.filter(task=>task.completed===false)?.length>0){
      setToDoHeading('Список задач')
    }else{
      setToDoHeading('Нет задач в списке')
    }
  },[
